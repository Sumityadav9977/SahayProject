...
export default function UserPage() {
 ...
 const [filterCompanyName, setFilterCompanyName] = useState('');
  ...
  const [companyData, setCompanyData] = useState([]); // State to store company details
  useEffect(() => {
    async function fetchCompanyData() {
      try {
        const response = await axios.post(`http://localhost:4000/exceldata/viewCompany/${companyName}`);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = companyData.map((n) => n.name);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Object.values(companyData).length) : 0;
  
  console.log('Type of companyData:', typeof companyData);
            ...
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                <UserListHead
                  ...
                  rowCount={Object.values(companyData).length}
                  numSelected={selected.length}
                  onRequestSort={handleRequestSort}
                  onSelectAllClick={handleSelectAllClick}
                />
                <TableBody>
                {Object.values(companyData).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
